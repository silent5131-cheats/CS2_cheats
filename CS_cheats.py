    ...
    def main(settings):
        ...
        while True:
            try:
                ...
                if keyboards == "XButton1":
                    keyboards = 0x05
                elif keyboards == "XButton":
                    keyboards = 0x04
                elif keyboards == "XButton2":
                    keyboards = 0x06

                if win32api.GetAsyncKeyState(keyboards):
...
def aim():
    ...
    def aimbot(target_list, radius, aim_mode_distance, window_size):
        ...
        max_delta_z = -float('inf')  

       
        pixel_radius = float('inf')  
        if radius > 0:
        ...
        for target_info in target_list:
            ...
            if dist_2d <= pixel_radius:
                ...
                if aim_mode_distance == 1:  
                    current_deltaZ = target_info.get('deltaZ', -1)  
    ...
    def main(settings):
        ...
        if window_size == (None, None) or window_size[0] is None or window_size[1] is None:
            print("Error: Failed to get CS2 window size. Exiting aim thread")
            return 
        ...
        try:
            ...
            print("Error: Process cs2.exe not found while starting aim thread.")
            return
